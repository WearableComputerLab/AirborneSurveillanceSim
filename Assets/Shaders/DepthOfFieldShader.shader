//Stolen from https://catlikecoding.com/unity/tutorials/advanced-rendering/depth-of-field/
//Modifications: Added "infinity"
...
Shader "Hidden/DepthOfField" {
	...
	CGINCLUDE
		...
		float _BokehRadius, _FocusDistance, _FocusRange, _InfinityLow, _InfinityHigh, _DepthScale;
		...
		Pass { // 0 circleOfConfusionPass
			CGPROGRAM
				...
				half FragmentProgram (Interpolators i) : SV_Target {
					...
					//if depth < infHigh, proceed normally
					//otherwise, smoothStep _FocusDistance from infLow to infHigh
					
					if(depth < _InfinityHigh) {
					    float coc = (depth - _FocusDistance) / _FocusRange;
					    coc = clamp(coc, -1, 1) * _BokehRadius;
					    
					    return coc;
					} else {
					    return (1.0 - smoothstep(_InfinityLow, _InfinityHigh, _FocusDistance)) * _BokehRadius;
					}
				}
			ENDCG
		}
